...
<section id="about" class="pt-8 sm:pt-10 pb-16 sm:pb-20 bg-white"
         x-data="{ shown: false }"
    ...
    <div class="max-w-[1760px] mx-auto px-4 sm:px-8 lg:px-12">
        ...
        <h2 class="section-heading">{{ __('Everything You Need, Fresh Every Day') }}</h2>
        <p class="text-center text-maroon-500 mt-4 mb-10 sm:mb-14 max-w-2xl mx-auto">
            {{ __('From daily essentials to festive treats — fresh stock delivered to your door every morning.') }}
        </p>
        ...
        <div class="space-y-4 sm:space-y-5">
            ...
            <a href="{{ route('products.index', ['category' => 'sweets']) }}"
               ...
               :class="shown ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'">
                <img src="{{ asset('images/ranges/all-mithai.jpg') }}" alt="{{ __('Grocery range') }}" loading="lazy"
                ...
                <div class="relative z-10 h-full flex items-center px-6 sm:px-10 lg:px-14">
                    <div class="max-w-sm">
                        <p class="text-gold-300 font-semibold tracking-widest uppercase text-[10px] sm:text-xs mb-2 sm:mb-3">{{ __('Most Loved') }}</p>
                        <h3 class="font-display text-2xl sm:text-4xl font-bold text-cream leading-tight drop-shadow-md mb-2 sm:mb-3">{{ __('Fresh Picks, Stocked Daily') }}</h3>
                        <p class="text-xs sm:text-base text-gold-100/90 mb-4 sm:mb-6">{{ __('Pure ingredients, zero shortcuts, every single batch.') }}</p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5">
                @foreach ([
                    ['image' => 'namkeen.jpg', 'title' => __('Fast & Fresh Delivery'), 'text' => __('Groceries at your door in minutes'), 'category' => 'namkeen'],
                    ['image' => 'cookies.jpg', 'title' => __('Cookies & Bakes'), 'text' => __('Buttery, always fresh'), 'category' => 'cookies'],
                    ['image' => 'desi-ghee.jpg', 'title' => __('Household & Personal Care'), 'text' => __('Everything for home & hygiene'), 'category' => 'desi-ghee'],
                ] as $i => $range)
