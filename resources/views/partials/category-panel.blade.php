...
<div x-data="{ open: false }" @open-category-panel.window="open = true" class="lg:hidden">
    <div x-show="open" x-cloak class="fixed inset-0 z-[105]">
        ...
        <div x-show="open"
             ...
             class="absolute inset-x-0 bottom-0 bg-ivory rounded-t-3xl shadow-2xl max-h-[80vh] flex flex-col will-change-transform overflow-hidden">
            {{-- warm dot-grid texture + soft ambient glow blobs, same "ambient ochre glow"
                 language already used on /products and the bestseller carousel — keeps this
                 panel from reading as a flat, empty sheet without competing with the tiles --}}
            <div class="absolute inset-0 bg-dot-grid text-gold-400/25 pointer-events-none"></div>
            <div class="absolute -top-10 -left-8 w-40 h-40 rounded-full bg-gold-400/20 blur-3xl pointer-events-none"></div>
            <div class="absolute -bottom-12 -right-10 w-48 h-48 rounded-full bg-maroon-400/10 blur-3xl pointer-events-none"></div>

            <div class="relative pt-3 pb-1 flex justify-center shrink-0" @click="open = false">
                <span class="w-10 h-1.5 rounded-full bg-gold-300"></span>
            </div>
            <div class="relative flex items-center justify-between px-5 py-3 border-b border-gold-200/60 shrink-0">
            ...
            <div class="relative flex-1 overflow-y-auto px-5 py-5 pb-[max(1.25rem,env(safe-area-inset-bottom))]">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        @foreach ($categories as $i => $category)
                            <a href="{{ route('products.index', ['category' => $category->slug]) }}"
                               class="animate-rise-in flex flex-col items-center gap-2 text-center group"
                               style="animation-delay: {{ min($i, 9) * 60 }}ms">
                                <span class="relative w-20 h-20 sm:w-24 sm:h-24 rounded-full border-2 border-gold-400 shadow-sm overflow-hidden transition duration-200 group-active:scale-95 group-hover:border-maroon-600 group-hover:shadow-md">
