...
@section('title', 'Festival Special')
@section('page-title', 'Festival Special')

@section('content')
    <p class="text-maroon-500 text-sm mb-5 max-w-2xl">
        Pick which products show up in the <span class="font-semibold">Festival Special</span> section on the homepage. Unchecked products stay in the catalog but won't appear in that section.
    </p>

    <form method="POST" action="{{ route('admin.festival-special.update') }}" x-data="{ dirty: false, q: '' }" @change="dirty = true">
        @csrf
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 mb-4 sticky top-0 z-10 bg-cream/95 backdrop-blur py-2 -mx-2 px-2">
            <div class="flex items-center gap-3 flex-wrap">
                <p class="text-sm text-maroon-500"><span class="font-semibold text-maroon-700">{{ $products->where('is_festival_special', true)->count() }}</span> of {{ $products->count() }} selected</p>
                    ...
                    <div class="bg-white rounded-xl border border-gold-200/60 divide-y divide-gold-50">
                        @foreach ($group as $product)
                            <label class="flex items-center gap-4 px-5 py-3 cursor-pointer hover:bg-cream/50 transition"
                                ...
                                <input type="checkbox" name="product_ids[]" value="{{ $product->id }}" @checked($product->is_festival_special)
