...
<section class="relative py-14 bg-ivory min-h-[70vh]">
    <div class="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <h1 class="font-display text-3xl sm:text-4xl text-maroon-800">{{ __('Checkout') }}</h1>
        ...
        <div x-cloak x-data='checkoutPage(@json($cartItemsForJs), @json($appliedCoupon), @json($addresses), @json($defaultName), @json($defaultPhone), @json($reward), @json($buyNowProductId), @json($buyNowQuantity), @json($availableCoupons), @json($buyNowPortion))'>
            <div class="grid lg:grid-cols-[1fr_380px] gap-8 mt-8 items-start">
                <div class="space-y-5">
                    {{-- review items --}}
                    <div class="bg-white rounded-2xl border border-gold-200/60 shadow-sm p-6">
                        ...
                        <div class="mt-4 divide-y divide-gold-100">
                            <template x-for="item in items" :key="item.id">
                                <div class="flex items-center gap-3.5 py-3">
                                    <img :src="item.image" :alt="item.name" class="w-14 h-14 rounded-lg object-cover border border-gold-200/60 shrink-0">
                                    <div class="flex-1 min-w-0">
                                        <p class="text-maroon-800 font-medium truncate" x-text="item.name"></p>
                                        <p class="text-xs text-maroon-400" x-text="(item.type === 'loose' ? portionLabel(item.portion) : item.weight) + ' × ' + item.quantity"></p>
                                    </div>
                                    <p class="text-maroon-800 font-semibold shrink-0">₹<span x-text="item.price * item.quantity"></span></p>
                                </div>
                            </template>
                        </div>
                    </div>
                        ...
                        <div class="grid sm:grid-cols-2 gap-3 mt-4">
                            <div>
                                ...
                                <input type="text" x-model="customerName" placeholder="{{ __('Who is this order for?') }}"
                                       class="w-full rounded-lg border border-gold-300/70 bg-cream/60 px-3 py-2 text-sm text-maroon-800 placeholder-maroon-300 focus:outline-none focus:ring-2 focus:ring-gold-400 focus:border-gold-400 transition">
                            </div>
                            <div>
                    ...
                    <div class="bg-white rounded-2xl border border-gold-200/60 shadow-sm p-6">
                        ...
                        <div class="mt-4 space-y-2.5" x-show="addresses.length > 0">
                            <template x-for="addr in addresses" :key="addr.id">
                                <div @click="selectAddress(addr)"
                                     class="relative rounded-xl border p-3.5 cursor-pointer transition"
                                    ...
                                    <div class="flex items-start justify-between gap-3">
                                        <div class="min-w-0">
                                            <p class="text-sm font-semibold text-maroon-800 flex items-center gap-2">
                                            ...
                                            <p class="text-xs text-maroon-500 mt-1 truncate" x-text="addr.address_line"></p>
                                        </div>
                                        <div class="flex items-center gap-2.5 shrink-0 text-xs">
                                            <button type="button" x-show="!addr.is_default" x-cloak @click.stop="makeDefault(addr)" class="text-maroon-400 hover:text-gold-600 underline transition">{{ __('Make default') }}</button>
                                            <button type="button" @click.stop="deleteAddress(addr)" aria-label="Delete address" class="text-maroon-300 hover:text-red-600 transition">
                            ...
                            <button type="button" x-show="!showNewAddressForm" @click="openNewAddressForm()"
                                    class="text-sm font-semibold text-gold-600 hover:text-gold-700 underline underline-offset-2 transition">
                        ...
                        <div x-show="showNewAddressForm" x-cloak class="mt-4" :class="addresses.length > 0 && 'pt-4 border-t border-gold-100'">
                            ...
                            <textarea x-model="newAddressText" @input="checkoutError = ''" rows="3"
                                      ...
                                      class="w-full rounded-lg border border-gold-300/70 bg-cream/60 px-3 py-2 text-sm text-maroon-800 placeholder-maroon-300 focus:outline-none focus:ring-2 focus:ring-gold-400 focus:border-gold-400 transition"></textarea>
                            ...
                            <div x-show="!$store.shop.restricted" x-cloak class="mt-3">
                                <div x-show="locationStatus === 'idle'" x-cloak class="rounded-lg bg-gold-100/60 border border-gold-300/60 text-maroon-700 text-xs px-3.5 py-2.5 flex items-center justify-between gap-3">
                                    <span>📍 {{ __('Share your exact location to help our delivery rider find you faster.') }}</span>
                                    <button @click="captureLocationOptional()" type="button" class="shrink-0 font-semibold text-gold-600 hover:text-gold-700 underline underline-offset-2 transition">
                                        {{ __('Share location') }}
                                    </button>
                                </div>
                                <div x-show="locationStatus === 'checking'" x-cloak class="rounded-lg bg-gold-100/60 border border-gold-300/60 text-maroon-700 text-xs px-3.5 py-2.5">
                ...
                <div class="bg-white rounded-2xl border border-gold-200/60 shadow-sm p-6 lg:sticky lg:top-28">
                    ...
                    <div class="border-t border-gold-200/70 mt-4 pt-4 flex items-center justify-between">
                        ...
                        <span class="font-display font-semibold text-2xl text-maroon-800">₹<span x-text="total()"></span></span>
                    </div>
                            ...
                            <div x-transition:enter="transition ease-out duration-300"
                                ...
                                <form @submit.prevent="applyCoupon()" class="flex gap-2">
                                    <input type="text" x-model="couponCode" @input="couponError = ''"
                                           ...
                                           class="flex-1 min-w-0 rounded-lg border border-gold-300/70 bg-cream/60 px-3 py-2 text-sm text-maroon-800 placeholder-maroon-300 uppercase tracking-wide focus:outline-none focus:ring-2 focus:ring-gold-400 focus:border-gold-400 transition">
